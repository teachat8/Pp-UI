    ...
    methods : {
        init() {
            ...
            if( this.num<=1 ) {
                ...
                // 绑定手指触摸事件
                this.swiperDom.addEventListener('touchstart', (event) => {
                    this.startListener(event);
                });

                // 绑定手指滑动事件            
                this.swiperDom.addEventListener('touchmove', (event) => {
                    if ( this.oneEnd ) {
                ...
                // 绑定手指结束滑动事件            
                this.swiperDom.addEventListener('touchend', (event) => {
                    this.endListener(event);               
                });

                // 绑定TransitionEnd事件 
                this.swiperDom.addEventListener('webkitTransitionEnd', (event) => {
                    this.oneEnd = false;
